 ...
 *  The goal of this new structure is to avoid to swap nodes in the main array.
 */
typedef struct {
    void *A; //This is the array used to store heap nodes
    int *key_pos; //This array will store the position of the nodes
    int *rev_pos; //This array will store the nodes on a given position
    unsigned int num_of_elem; //this is the number of nodes in the heap
    unsigned int max_size; //max number of nodes, size of A
    size_t key_size; //size of the key type
    total_order_type leq; //the heap total order
    void *max_order_value; //pointer to a mem location that stores the max wrt the order (the maximum possible type).
} binheap_type;
...
/**********************************************************************
 * Fixes the heap properties of a (broken) heap.
 *
 * This function rearranges the nodes in the heap so that each parent
 * is lower than the its childern w.r.t to the total order of the heap.
 *  
 * @param H is the heap to be fixed.
 **********************************************************************/
